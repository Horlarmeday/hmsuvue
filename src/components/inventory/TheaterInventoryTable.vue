...
export default {
  ...
  data() {
    return {
      ...
      meta: '',
      items: [],
      itemsUrl: '/inventory/theater',
      loading: false
    }
  },
  mounted() {
    this.getTheaterItems()
  },
  methods: {
    handleError(error) {
      this.$iziToast.error({
      ...
      axios
        ...
        .then(response => {
          ...
          for (let i = 0; i < items.length; i++) {
            items[i].url = '/edit-theater/' + items[i]._id
          }
        })
        .catch(error => {
